...
from .database import init_db, close_db, get_driver
...
SERVICE_NAME = "neo4j"  # change per service: mongo, neo4j, redis


def normalize_path(path: str) -> str:
    # neo4j element IDs first: 4:uuid:1
    path = re.sub(
        r"/\d+:[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}:\d+",
        "/{id}",
        path,
    )
...
async def create_user(user: schemas.UserCreate):
    return await crud.create_user(get_driver(), user)


@app.get("/users", response_model=list[schemas.UserResponse], tags=["Users"])
async def get_users(name: str | None = None):
    return await crud.get_users(get_driver(), name)


@app.get("/users/{user_id}", response_model=schemas.UserResponse, tags=["Users"])
async def get_user(user_id: str):
    return await crud.get_user(get_driver(), user_id)
...
async def update_user(user_id: str, data: schemas.UserUpdate):
    return await crud.update_user(get_driver(), user_id, data)


@app.delete("/users/{user_id}", status_code=204, tags=["Users"])
async def delete_user(user_id: str):
    await crud.delete_user(get_driver(), user_id)
...
async def get_product(product_id: str):
    return await crud.get_product(get_driver(), product_id)
...
async def update_product(product_id: str, data: schemas.ProductUpdate):
    return await crud.update_product(get_driver(), product_id, data)


@app.delete("/products/{product_id}", status_code=204, tags=["Products"])
async def delete_product(product_id: str):
    await crud.delete_product(get_driver(), product_id)
...
async def update_order(order_id: str, data: schemas.OrderUpdate):
    return await crud.update_order(get_driver(), order_id, data)


@app.delete("/orders/{order_id}", status_code=204, tags=["Orders"])
async def delete_order(order_id: str):
    await crud.delete_order(get_driver(), order_id)
